...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/20 13:27:07 by rtwitch           #+#    #+#             */
/*   Updated: 2021/11/20 16:45:36 by rtwitch          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	stat[fd] = make_line(fd, stat[fd]);
	if (!stat[fd])
		return (NULL);
	line = get_first_line(stat[fd]);
	stat[fd] = get_last_line(stat[fd]);
	return (line);
}
